...
function App() {
  ...
  const saveTeam = () => {
    ...
    if (game.onlineRoomCode && game.onlinePlayerId) {
      void (async () => {
        const response = await fetch(`/api/rooms/${game.onlineRoomCode}/roster`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ playerId: game.onlinePlayerId, roster: cleaned }) });
        if (!response.ok) return;
        const readyResponse = await fetch(`/api/rooms/${game.onlineRoomCode}/ready`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ playerId: game.onlinePlayerId }) });
