...
function GameView({ game, setGame, onReset }: { game:GameState; setGame:(g:GameState)=>void; onReset:()=>void }) {
  ...
  const [onlineError, setOnlineError] = useState('');
  useEffect(() => { if (!game.onlineRoomCode || !game.onlinePlayerId) return; const loadCards = async () => { try { const response = await fetch(`/api/rooms/${game.onlineRoomCode}/cards/${game.onlinePlayerId}?sessionToken=${encodeURIComponent(game.onlineSessionToken ?? '')}`); if (!response.ok) throw new Error('تعذر تحميل الكروت'); setCards((await response.json()).cards); } catch (error) { setOnlineError(error instanceof Error ? error.message : 'تعذر تحميل الكروت'); } }; void loadCards(); }, [game.onlineRoomCode, game.onlinePlayerId, game.onlineSessionToken]);
  useEffect(() => {
    if (!game.onlineRoomCode || !game.onlinePlayerId) return;
    let active = true;
    const sync = async () => {
      try {
        const response = await fetch(`/api/rooms/${game.onlineRoomCode}?playerId=${encodeURIComponent(String(game.onlinePlayerId))}&sessionToken=${encodeURIComponent(game.onlineSessionToken ?? '')}`);
  ...
  const reveal = () => { if (game.targetTeam===null || game.targetPlayer===null) return; const teams=game.teams.map((t,ti)=>ti===game.targetTeam?{...t,footballers:t.footballers.map((p,pi)=>pi===game.targetPlayer?{...p,revealed:true}:p)}:t); const nextState={...game,teams,phase:'reveal' as const,revealDecision:'choose' as const,revealSourcePlayer:game.targetPlayer,exclusionTargets:[]}; syncEvent('reveal', selected?.name, nextState); setGame(nextState); };
