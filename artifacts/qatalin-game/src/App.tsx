...
type GameState = { version: 2; screen: Screen; phase: Phase; playerCount: number; owners: string[]; teams: Team[]; setupIndex: number; round: number; turn: number; targetTeam: number | null; targetPlayer: number | null; exclusionTargets: number[]; revealDecision: 'choose' | 'exclude' | 'assassinate'; notes: string; winner: number | null; history: HistoryItem[]; onlineRoomCode?: string; onlinePlayerId?: string };
...
const initialState = (): GameState => ({ version: 2, screen: 'intro', phase: 'question', playerCount: 2, owners: ownerDefaults.slice(0, 2), teams: [], setupIndex: 0, round: 1, turn: 0, targetTeam: null, targetPlayer: null, exclusionTargets: [], revealDecision: 'choose', notes: '', winner: null, history: [] });
function readSavedGame(): GameState { try { const saved = JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? 'null'); return saved?.version === 2 ? { ...saved, exclusionTargets: saved.exclusionTargets ?? [], revealDecision: saved.revealDecision ?? 'choose' } : initialState(); } catch { return initialState(); } }
...
function GameView({ game, setGame, onReset }: { game:GameState; setGame:(g:GameState)=>void; onReset:()=>void }) {
  ...
  const reveal = () => { if (game.targetTeam===null || game.targetPlayer===null) return; const teams=game.teams.map((t,ti)=>ti===game.targetTeam?{...t,footballers:t.footballers.map((p,pi)=>pi===game.targetPlayer?{...p,revealed:true}:p)}:t); syncEvent('reveal', selected?.name); setGame({...game,teams,phase:'reveal',revealDecision:'choose',exclusionTargets:[]}); };
  const chooseRevealDecision = (decision:'exclude'|'assassinate') => { if (!selected?.isBoss || !selected.revealed) return; if (decision === 'assassinate') { act('assassinate'); return; } update({ revealDecision: decision, exclusionTargets: [] }); };
