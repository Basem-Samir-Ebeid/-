...
function RoomSetup({ game, setCount, update, onBack, onContinue }: { game: GameState; setCount: (n:number)=>void; update:(c:Partial<GameState>)=>void; onBack:()=>void; onContinue:()=>void }) {
  ...
  useEffect(() => { if(onlineMode !== 'lobby' || !roomCode || !playerId) return; let active = true; const load = async () => { try { const data = await requestJson(`${api}/rooms/${roomCode}?playerId=${encodeURIComponent(playerId)}`, { headers: { Authorization: `Bearer ${game.onlineSessionToken ?? ''}` } }); if (active) { const players = data.players ?? []; setOnlinePlayers(players); const me = players.find((player: { id?: string; displayName:string; teamName:string }) => player.id === playerId); if (me) { setName(me.displayName); setTeamName(me.teamName); } } } catch (error) { if (active) setOnlineError(error instanceof Error ? error.message : 'تعذر تحديث الغرفة'); } }; void load(); const timer=window.setInterval(load, 2500); return () => { active = false; window.clearInterval(timer); }; }, [api, onlineMode, roomCode, playerId, game.onlineSessionToken]);
