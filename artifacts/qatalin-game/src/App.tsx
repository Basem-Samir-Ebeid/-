...
function App() {
  ...
  const saveTeam = () => {
    ...
    if (game.onlineRoomCode && game.onlinePlayerId) {
      void fetch(`/api/rooms/${game.onlineRoomCode}/roster`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ playerId: game.onlinePlayerId, roster: cleaned }) });
...
function GameView({ game, setGame, onReset }: { game:GameState; setGame:(g:GameState)=>void; onReset:()=>void }) {
  ...
  return <motion.main className="game-wrap" initial={{opacity:0}} animate={{opacity:1}}><div className="game-header"><div><div className="eyebrow">دور {attacker.owner}</div><h1>{game.phase==='question'?'غرفة التحقيق':game.phase==='target'?'حدد هدفك':game.phase==='assassination'?'اختر لاعباً للاغتيال':'تم كشف الهوية'}</h1></div><div className="round-marker"><strong>{game.round}</strong> الجولة الحالية</div></div><div className="turn-banner"><Target /><div><strong>{attacker.owner} يهاجم الآن</strong><span>اختر لاعباً من فريق خصمك. لا يمكنك استهداف فريقك.</span></div></div><div className="phase-layout"><section className="paper-card phase-card">
    ...
    {game.phase==='reveal'&&selected&&<><div className={`reveal-result ${selected.isBoss?'is-boss':''}`}><div className="result-symbol">{selected.isBoss?<Skull />:<UserRound />}</div><span>تم كشف</span><h2>{selected.name}</h2><strong>{selected.isBoss?'زعيم عصابة':'لاعب عادي'}</strong><p>{selected.isBoss?'تم كشف زعيم الخص��. قبل اختيار أي لاعب، حدّد قرارك أولاً: الاستبعاد أو الاغتيال المباشر.':'ليس زعيماً. يمكنك استبعاده من قائمة الخصم.'}</p></div><div className="decision-actions">{selected.isBoss?game.revealDecision==='choose'?<div className="reveal-choice"><strong>اختر لاعباً آخر من نفس الفريق لاستبعاده مع الزعيم المكتشف</strong><div className="reveal-choice-actions"><button className="primary-button" onClick={chooseRevealDecision}><UserRound /> اختر لاعباً لاستبعاده مع الزعيم</button></div></div>:null:<button className="primary-button" onClick={()=>act('exclude')}><UserRound /> استبعاد اللاعب</button>}</div></>}
