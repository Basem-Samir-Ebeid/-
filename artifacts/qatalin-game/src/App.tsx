...
function GameView({ game, setGame, onReset }: { game:GameState; setGame:(g:GameState)=>void; onReset:()=>void }) {
  ...
  const chooseRevealDecision = (decision:'exclude'|'assassinate') => { if (!selected?.isBoss || !selected.revealed) return; update({ revealDecision: decision, phase: 'assassination', targetPlayer: null, exclusionTargets: [] }); };
